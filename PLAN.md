...
- [x] 2026-03-17: Aligned the height of the clear filter button with the user and label filter buttons in `apps/web/core/components/cycles/kpi/page-shell.tsx` by explicitly setting standard internal padding `py-1` and `px-2.5` on the elements directly instead of using external fixed-height bounds which interacted inconsistently with the dropdown wrappers. Touched files: `apps/web/core/components/cycles/kpi/page-shell.tsx`, `PLAN.md`.
...
- [x] 2026-03-17: `pnpm --filter web exec eslint "core/components/cycles/kpi/page-shell.tsx"` passed after applying uniform paddings to filter buttons.
- [x] 2026-03-17: `pnpm --filter web check:types` passed after applying uniform paddings to filter buttons.
